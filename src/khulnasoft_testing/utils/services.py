...
from khulnasoft_testing.constants.daemons import CLUSTER_DAEMON, API_DAEMON, KHULNASOFT_AGENT, KHULNASOFT_MANAGER, KHULNASOFT_AGENT_WIN
from khulnasoft_testing.constants.paths.binaries import BIN_PATH, KHULNASOFT_CONTROL_PATH
from khulnasoft_testing.constants.paths.sockets import KHULNASOFT_SOCKETS, KHULNASOFT_OPTIONAL_SOCKETS
from khulnasoft_testing.constants.paths.variables import VAR_RUN_PATH, VERSION_FILE
from khulnasoft_testing.constants.platforms import MACOS, SOLARIS, WINDOWS
...
def get_service() -> str:
    """
    Retrieves the name of the Khulnasoft service running on the current platform.

    Returns:
        str: The name of the Khulnasoft service. It can be either 'khulnasoft-manager'
             or 'khulnasoft-agent'.
    ...
    if platform.system() in ['Windows', WINDOWS]:
        return KHULNASOFT_AGENT

    else:  # Linux, sunos5, darwin, aix...
        service = subprocess.check_output([
            KHULNASOFT_CONTROL_PATH, "info", "-t"
        ], stderr=subprocess.PIPE).decode('utf-8').strip()

    return KHULNASOFT_MANAGER if service == 'server' else KHULNASOFT_AGENT


def get_version() -> str:
    """
    Retrieves the version of the Khulnasoft software installed on the current platform.

    Returns:
        str: The version of the Khulnasoft software.
...
def control_service(action, daemon=None, debug_mode=False):
    ...
    Args:
        ...
        daemon (str, optional): Name of the daemon to be controlled. None for the whole Khulnasoft service. Default `None`.
        ...
        if action == 'restart':
            ...
            for _ in range(error_109_windows_retry):
                command = subprocess.run(
                    ["net", action, "KhulnasoftSvc"], stderr=subprocess.PIPE)
                result = command.returncode
                if result != 0:
                    if action == 'stop' and 'The Khulnasoft service is not started.' in command.stderr.decode():
                        result = 0
                        break
        ...
        if daemon is None:
            if sys.platform == MACOS or sys.platform == SOLARIS:
                result = subprocess.run(
                    [KHULNASOFT_CONTROL_PATH, action]).returncode
            else:
                result = subprocess.run(
            ...
            if action == 'restart':
                ...
                sockets.delete_sockets(KHULNASOFT_SOCKETS[daemon])
            else:
...
def check_daemon_status(target_daemon=None, running_condition=True, timeout=10, extra_sockets=[]):
    """Wait until Khulnasoft daemon's status matches the expected one. If timeout is reached and the status didn't match,
       it raises a TimeoutError.

    Args:
        target_daemon (str, optional):  Khulnasoft daemon to check. Default `None`. None means all.
    ...
    while elapsed_time < timeout and not condition_met:
        if sys.platform == WINDOWS:
            condition_met = check_if_process_is_running(KHULNASOFT_AGENT_WIN) == running_condition
        else:
            control_status_output = subprocess.run([KHULNASOFT_CONTROL_PATH, 'status'],
            ...
            for lines in control_status_output.splitlines():
                ...
                if current_daemon == target_daemon or target_daemon is None:
                    if current_daemon in KHULNASOFT_SOCKETS.keys():
                        socket_set = {path for path in KHULNASOFT_SOCKETS[current_daemon]}
                    else:
                        socket_set = set()
                    # We remove optional sockets and add extra sockets to the set to check
                    socket_set.difference_update(KHULNASOFT_OPTIONAL_SOCKETS)
