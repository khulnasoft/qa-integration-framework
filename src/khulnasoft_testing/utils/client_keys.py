...
from khulnasoft_testing.utils import file
...
def add_client_keys_entry(agent_id, agent_name, agent_ip='any', agent_key=None) -> None:
    ...
    with open(KHULNASOFT_CLIENT_KEYS_PATH, 'r') as client_keys:
    ...
    with open(KHULNASOFT_CLIENT_KEYS_PATH, 'w') as client_keys:
...
def get_client_keys(path: str = KHULNASOFT_CLIENT_KEYS_PATH) -> List[dict]:
    """Get client keys from a file.

    Args:
        path (str, optional): Path to the file containing the client keys.
            Defaults to KHULNASOFT_CLIENT_KEYS_PATH.
