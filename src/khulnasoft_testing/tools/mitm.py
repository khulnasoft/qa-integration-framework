...
from khulnasoft_testing.constants.users import KHULNASOFT_UNIX_USER, KHULNASOFT_UNIX_GROUP
...
class StreamHandler(socketserver.BaseRequestHandler):
    ...
    def default_khulnasoft_handler(self):
        """Default khulnasoft daemons TCP handler method for MITM server."""
    ...
    def handle(self):
        ...
        if self.server.mitm.handler_func is None:
            self.default_khulnasoft_handler()
        else:
            while not self.server.mitm.event.is_set():
...
class DatagramHandler(socketserver.BaseRequestHandler):
    ...
    def default_khulnasoft_handler(self):
        """Default khulnasoft daemons UDP handler method for MITM server."""
        data = self.request[0]
...
class ManInTheMiddle:
    ...
    def run(self, *args):
        ...
        if isinstance(self.listener_socket_address, str):
            uid = pwd.getpwnam(KHULNASOFT_UNIX_USER).pw_uid
            gid = grp.getgrnam(KHULNASOFT_UNIX_GROUP).gr_gid
