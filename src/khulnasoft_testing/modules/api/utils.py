...
from khulnasoft_testing import session_parameters
from khulnasoft_testing.constants.api import KHULNASOFT_API_PROTOCOL, KHULNASOFT_API_HOST, KHULNASOFT_API_PORT, KHULNASOFT_API_USER, \
                                        KHULNASOFT_API_PASSWORD, LOGIN_ROUTE, USERS_ROUTE, RESOURCE_ROUTE_MAP, \
                                        TARGET_ROUTE_MAP
from khulnasoft_testing.constants.paths.api import KHULNASOFT_API_CERTIFICATE
from khulnasoft_testing.modules.api.patterns import API_LOGIN_ERROR_MSG
from khulnasoft_testing.utils.file import read_json_file
...
def get_base_url(protocol: str = KHULNASOFT_API_PROTOCOL, host: str = KHULNASOFT_API_HOST, port: str = KHULNASOFT_API_PORT) -> str:
    """Get Khulnasoft API's base URL.
    ...
    Returns:
        base_url (str): Khulnasoft API's base URL
    """
...
def set_authorization_header(user: str = None, password: str = None) -> dict:
    ...
    """
    user = KHULNASOFT_API_USER if user is None else user
    password = KHULNASOFT_API_PASSWORD if password is None else password
...
def login(user: str = KHULNASOFT_API_USER, password: str = KHULNASOFT_API_PASSWORD,
          timeout: int = session_parameters.default_timeout, login_attempts: int = 1, sleep_time: int = 0,
          host: str = KHULNASOFT_API_HOST, port: str = KHULNASOFT_API_PORT, protocol: str = KHULNASOFT_API_PROTOCOL
    ...
    for _ in range(login_attempts):
        response = requests.post(url, headers=set_authorization_header(user, password), verify=KHULNASOFT_API_CERTIFICATE, timeout=timeout)
...
def allow_user_to_authenticate(user_id: str = None) -> requests.Response:
    ...
    response = requests.put(url + params, headers=authentication_headers, verify=KHULNASOFT_API_CERTIFICATE)
...
def manage_security_resources(method: str = 'get', resource: Union[dict, str] = None,
    ...
    authentication_headers = login()[0]
    response = getattr(requests, method)(url, headers=authentication_headers, verify=KHULNASOFT_API_CERTIFICATE, json=payload)
...
def relate_resources(test_metadata: dict) -> None:
    ...
    for origin in relationships:
        ...
        for idx, target_value in enumerate(target_values):
            ...
            response = requests.post(url, headers=login()[0], verify=KHULNASOFT_API_CERTIFICATE)
            if response.status_code != 200 or response.json()['error'] != 0:
...
def remove_resources_relationship(origin_resource: dict = None, target_resource: dict = None) -> dict:
    ...
    response = requests.delete(url, headers=login()[0], verify=KHULNASOFT_API_CERTIFICATE)
    if response.status_code != 200 or response.json()['error'] != 0:
